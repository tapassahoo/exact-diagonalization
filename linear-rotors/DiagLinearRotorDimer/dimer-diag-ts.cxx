...
int main(int argc,char **argv) 
{
    ...
    int size_theta1 = 10;//2.*jmax+5;
	int size_theta2 = 10;//2.*jmax+5;
    int size_phi1   = 21;//2.*(2.*jmax+7); // 
    int size_phi2   = 21;//2.*(2.*jmax+7); // 
...
//==============new segment=================//
    double uvec1[sizej1j2m1m2];
    double psi[size_theta1*size_theta2*size_phi1*size_phi2];
    int ii=0;
    for (int i = 0; i < size_theta1; i++)
    {
        theta1 = grid_theta1(i);
        			...
        			for (int j1 = 0; j1 <= jmax; j1++)
        			{
            			...
            			for (int m1 = -j1; m1 <= j1; m1++) 
            			{
                			for (int j2 = 0; j2 <= jmax; j2++)
                			{
                    			...
                    			for (int m2 = -j2; m2 <= j2; m2++) 
                    			{
	       double sumpRe  = basisfunctionPRe(m1,weights_phi1(k),phi1)*basisfunctionPRe(m2,weights_phi2(l),phi2);
           sumpRe += -basisfunctionPIm(m1,weights_phi1(k),phi1)*basisfunctionPIm(m2,weights_phi2(l),phi2);
		   sumpRe /= sqrt(weights_phi1(k)*weights_phi2(l));

	   	   double sumpIm  = basisfunctionPIm(m1,weights_phi1(k),phi1)*basisfunctionPRe(m2,weights_phi2(l),phi2);
           sumpIm += basisfunctionPRe(m1,weights_phi1(k),phi1)*basisfunctionPIm(m2,weights_phi2(l),phi2);
		   sumpIm /= sqrt(weights_phi1(k)*weights_phi2(l));

	       double sumtRe  = sumpRe*basisPjm(j1,m1,weights_theta1(i),cos(theta1))*basisPjm(j2,m2,weights_theta2(j),cos(theta2));
           sumtRe /= sqrt(weights_theta1(i)*weights_theta2(j));
	       double sumtIm  = sumpIm*basisPjm(j1,m1,weights_theta1(i),cos(theta1))*basisPjm(j2,m2,weights_theta2(j),cos(theta2));
           sumtIm /= sqrt(weights_theta1(i)*weights_theta2(j));

           uvec1[index_j1j2m1m2] = HRe(index_j1j2m1m2,0);
           sum    += uvec1[index_j1j2m1m2]*sumtRe;
                                index_j1j2m1m2++;
        						}
 							}
						}
    ...
    string fname1 = "DataForHistogramFor2HF-DipoleMoment" + bc.str()+".txt";
    ...
    double sum = 0.0;
    ii = 0;
    double density[size_theta1*size_theta2*size_phi1*size_phi2];
    double xtilde[size_theta1*size_theta2*size_phi1*size_phi2];
    for (int i = 0; i < size_theta1; i++)
    {
        for (int j = 0; j < size_theta2; j++)
        {
            for (int k = 0; k < size_phi1; k++)
            {
                for (int l = 0; l < size_phi2; l++)
                {
        			sum += psi[ii]*psi[ii];//*sqrt(weights_theta1(i)*weights_theta2(j)*weights_phi1(k)*weights_phi2(l));
        			density[ii] = psi[ii]*psi[ii];
        			histogram<< setw(20)<<setfill(' ')<<Angle_V(i*size_theta2+j, k*size_phi2+l);
        			histogram<< setw(20)<<setfill(' ')<<density<<"   "<<psi[ii]<<endl;
                    xtilde[ii] = Angle_V(i*size_theta2+j, k*size_phi2+l);
                    ii++;
                }
            }
        }
...
//==========================bins============================//
    ...
    for (int i = 0; i < numbins - 1; i++)
    {
        ...
        for (int j = 0; j < size_theta1*size_theta2*size_phi1*size_phi2; j++)
        {
            if (xtilde[j] >= binsi && xtilde[j] <= binsf) hist[i]+=density[j];
        }
