...
int main(int argc,char **argv) {
    double sump, sumt1, sumt2, sum, sum1;
    int ii, jj, kk;
	...
	for (int i = 0; i < size_theta1; i++)
    {
        ...
        for (int j = 0; j < size_theta2; j++)
        {
            ...
            for (int k = 0; k < size_phi; k++)
            {
                ...
                density(kk) = sum*sum;
				sump += density(kk)*weights_phi(k);
            }
            sumt2 += sump*weights_theta2(j);
        }
        sumt1 += sumt2*weights_theta1(i);
    }
    logout <<"Norm checking for Ground State "<<sumt1 <<endl;

//=============================Distribution cos(theta1)=====================================//
    ofstream outdens1("DensityCosTheta1.txt");
    outdens1.precision(10);
    outdens1.setf(ios::right);
    outdens1 << showpoint;
    outdens1 << setw(20)<<setfill(' ');
	...
	for (int i = 0; i < size_theta1; i++)
    {
        ...
        sumt1 += sumt2*weights_theta1(i);
        outdens1<< setw(20)<<setfill(' ')<<cos(theta1);
        outdens1<< setw(20)<<setfill(' ')<<sumt2<<endl;
    }

    logout <<"Norm checking for Ground State cost1 "<<sumt1 <<endl;
//=============================Distribution cos(theta2)=====================================//
    ofstream outdens2("DensityCosTheta2.txt");
    outdens2.precision(10);
    outdens2.setf(ios::right);
    outdens2 << showpoint;
    outdens2 << setw(20)<<setfill(' ');
	...
	for (int j = 0; j < size_theta2; j++)
    {
        ...
        for (int i = 0; i < size_theta1; i++)
        {
            ...
            for (int k = 0; k < size_phi; k++)
            {
                kk = k + (j + i*size_theta2)*size_phi;
                sump += density(kk)*weights_phi(k);
            }
            sumt1 += sump*weights_theta1(i);
        }
        outdens2<< setw(20)<<setfill(' ')<<cos(theta2);
        outdens2<< setw(20)<<setfill(' ')<<sumt1<<endl;
        sumt2 += sumt1*weights_theta2(j);
    }

    logout <<"Norm checking for Ground State cost2 "<<sumt2 <<endl;
//========================Distribution Phi==============================//
    ofstream outdens3("DensityPhi.txt");
    outdens3.precision(10);
    outdens3.setf(ios::right);
    outdens3 << showpoint;
    outdens3 << setw(20)<<setfill(' ');
    ...
    for (int k = 0; k < size_phi; k++)
    {
    	...
    	for (int i = 0; i < size_theta1; i++)
        {
            ...
            {
                kk = k + (j + i*size_theta2)*size_phi;
                sumt2 += density(kk)*weights_theta2(j);
            }
            sumt1 += sumt2*weights_theta1(i);
        }
        outdens3<< setw(20)<<setfill(' ')<<phi;
        outdens3<< setw(20)<<setfill(' ')<<sumt1<<endl;
        sump += sumt1*weights_phi(k);
    }
    logout <<"Norm checking for Ground State phi "<<sump <<endl;
//==========================Histogram relative angle============================//
    ...
    ofstream outdens4("DensityCosThetaTilde.txt");
    outdens4.precision(10);
    outdens4.setf(ios::right);
    outdens4 << showpoint;
    outdens4 << setw(20)<<setfill(' ');
    ...
    for (int ibin = 0; ibin < numbins; ibin++) 
    {
        ...
        outdens4<< setw(20)<<setfill(' ')<<bins;
        outdens4<< setw(20)<<setfill(' ')<<hist[ibin]/sum1<<endl;
    }
    logout<<"Normalized Histogram "<<sum<<endl;
