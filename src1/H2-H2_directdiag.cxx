...
int main(int argc,char **argv) {
  ...
  if (argc != 3) {
    cerr<<"usage: "<<argv[0]<<" <R value> in Angstroms <size nu_total> 1, 2, or 4"<<endl;
    exit(0);
  }
  ...
  int sizej=2; // for different sized j basis for each monomer, you must modify the fbasisT1 and fbasisT2 fxns such that msize is determined by the largest j basis and the smaller j basis skips m > 2*j
  // With odd j the potential part of hamiltonian contributes no off-diagonal elements (that would affect the lowest eigen value after diagonalization) due to symmetry in the expectation value integration with legendre polynomials, so odd j will be skipped
	...
	for (j1=abs(m1);j1<=jmax;j1++){
	  ...
	  for (j2=abs(m2);j2<=jmax;j2++){
		...
		for (j2p=abs(m2p);j2p<=jmax;j2p++){
		  ...
		  // diagonal part
		  if (j1==j1p && j2==j2p &&  n1==n1p && n2==n2p && m1==(m1p))
		    delta(nut*sizej1j2m+index_jm12,nut*sizej1j2m+index_jm12)+=.5;

		  // P12
		  if (j1==j2p && j2==j1p && m1==(-m1p) && n1==n2p && n2==n1p)
		  //if (j1==j2p && j2==j1p &&  n1==n2p && n2==n1p)
  ...
  if (size_nu==2) 
    cout<<Rpt*BohrToA<<"  "<<ev(0)-Enuj(nulist1[0]*sizej+0)-Enuj(nulist2[0]*sizej+0)<<" "<<ev(1)-Enuj(nulist1[0]*sizej+0)-Enuj(nulist2[0]*sizej+0)<<" "<<evsym(0)-Enuj(nulist1[0]*sizej+0)-Enuj(nulist2[0]*sizej+0)<<" "<<evsym(1)-Enuj(nulist1[0]*sizej+0)-Enuj(nulist2[0]*sizej+0)<<endl;

  if (size_nu==4) 
    cout<<Rpt*BohrToA<<"  "<<ev(0)-E0j(0)-E0j(0)<<endl;
...
double PjNormal(int j,int m, double x)
{
  ...
  //  if (m<0)
  //  PjN=pow(-1.,mv)*exp(lgamma(jv-mv+1.)-lgamma(jv+mv+1.))*PjN; // refer to pg. 9 Angular Momentum, Richard N. Zare 
  return PjN;
