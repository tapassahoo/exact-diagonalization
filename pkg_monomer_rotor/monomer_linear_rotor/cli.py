...
def parse_arguments():
	...
	if args.molecule:
		...
		args.molecule = mol  # Normalize for later display

	# Validate presence of B constant
	if args.B_const is None:
		print(colored("[Error] Missing B constant. Use --B-const or --molecule.", "red"))
		sys.exit(1)

	# Determine potential strength V(θ)
	if args.potential_strength is None:
		if args.dipole_moment is None or args.electric_field is None:
			print(colored(
				"[Error] Provide --potential-strength or both --dipole-moment and --electric-field.",
				"red"
			))
			sys.exit(1)
