...
void HvMat(int jkem, int size_grid, matrix &Vpot, vector &basisJKeMRe, vector &basisJKeMIm, int inj1, int inj2, int inj1p, int inj2p, double *matre, double *matim);
...
int main(int argc,char **argv) 
{
	...
	int inj1p=2;
	int inj2p=2;
    size_theta = jmax+niter;
    size_phi   = 2*jmax+niter;
...
// Generation of names of output file //
	...
	string fname = "p-H2O-jmax" + jprefix.str()+"-Rpt"+rprefix.str() + "Angstorm-incr"+iterprefix.str()+"-grid"+thetaprefix.str()+"-"+phiprefix.str()+".txt";
...
//
	...
	matre = 0.0;
	matim = 0.0;
	HvMat(jkem, size_grid, Vpot, basisJKeMRe, basisJKeMIm, inj1, inj2, inj1p, inj2p, &matre, &matim);
	cout<<"TS "<<matre<<"  "<<matim<<endl;
	logout.close();
	exit(0);
}
...
void HvMat(int jkem, int size_grid, matrix &Vpot, vector &basisJKeMRe, vector &basisJKeMIm, int inj1, int inj2, int inj1p, int inj2p, double *matre, double *matim)
{
	...
	for (ig2=0;ig2<size_grid;ig2++) {
		for (ig1=0;ig1<size_grid;ig1++) {
			temp2re(ig2+ig1*size_grid)=(basisJKeMRe(ig1+i1*size_grid)*basisJKeMRe(ig2+i2*size_grid)-basisJKeMIm(ig1+i1*size_grid)*basisJKeMIm(ig2+i2*size_grid));
			temp2im(ig2+ig1*size_grid)=(basisJKeMRe(ig1+i1*size_grid)*basisJKeMIm(ig2+i2*size_grid)+basisJKeMIm(ig1+i1*size_grid)*basisJKeMRe(ig2+i2*size_grid));
		}
	}
	...
	double sum2=0.0;
	for (ig1=0; ig1<size_grid; ig1++) {
		//sum+=complex((temp1re(ig1+i2*size_grid)*basisJKeMRe(ig1+i1*size_grid)+temp1im(ig1+i2*size_grid)*basisJKeMIm(ig1+i1*size_grid)), (temp1im(ig1+i2*size_grid)*basisJKeMRe(ig1+i1*size_grid)-temp1re(ig1+i2*size_grid)*basisJKeMIm(ig1+i1*size_grid)));
		sum1+=temp1re(ig1+i2*size_grid)*basisJKeMRe(ig1+i1*size_grid)+temp1im(ig1+i2*size_grid)*basisJKeMIm(ig1+i1*size_grid);
		sum2+=temp1im(ig1+i2*size_grid)*basisJKeMRe(ig1+i1*size_grid)-temp1re(ig1+i2*size_grid)*basisJKeMIm(ig1+i1*size_grid);
	}
	*matre = sum1;
	*matim = sum2;
	return;
