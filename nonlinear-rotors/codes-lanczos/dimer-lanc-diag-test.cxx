...
//
	...
	for (int i=0; i<jkem; i++)
	{
		...
		{
			cout<<" i "<<i<<" j "<<j<<" basis "<<"("<<basisJKeMRe(j+i*size_grid)<<", "<<basisJKeMIm(j+i*size_grid)<<")"<<endl;
		}
	}
	exit(1);
	*/
	...
	*/
	...
	cvector u1(size_basis);

	/*
	for (int ig2=0;ig2<size_grid;ig2++) {
			cout<<Vpot(0,ig2)<<endl;
	}
	exit(1);
	*/
	...
	int i1p=2;
	int i2p=1;
	cout<<"L vec Rotor1: "<<jkemQuantumNumList(i1,0)<<" "<<jkemQuantumNumList(i1,1)<<" "<<jkemQuantumNumList(i1,2)<<endl;
	cout<<"L vec Rotor2: "<<jkemQuantumNumList(i2,0)<<" "<<jkemQuantumNumList(i2,1)<<" "<<jkemQuantumNumList(i2,2)<<endl;
	cout<<"R vec Rotor1: "<<jkemQuantumNumList(i1p,0)<<" "<<jkemQuantumNumList(i1p,1)<<" "<<jkemQuantumNumList(i1p,2)<<endl;
	cout<<"R vec Rotor2: "<<jkemQuantumNumList(i2p,0)<<" "<<jkemQuantumNumList(i2p,1)<<" "<<jkemQuantumNumList(i2p,2)<<endl;
	complex V_element=complex(0.0,0.0);
	for (int ig1=0;ig1<size_grid;ig1++) {
		for (int ig2=0;ig2<size_grid;ig2++) {
			V_element+=
			complex(basisJKeMRe(ig1+i1*size_grid),basisJKeMIm(ig1+i1*size_grid))*
			complex(basisJKeMRe(ig2+i2*size_grid),basisJKeMIm(ig2+i2*size_grid))*
			complex(Vpot(ig1,ig2),0.0)*
			complex(basisJKeMRe(ig1+i1p*size_grid),-basisJKeMIm(ig1+i1p*size_grid))*
			complex(basisJKeMRe(ig2+i2p*size_grid),-basisJKeMIm(ig2+i2p*size_grid));
		}
	}         
	cout<<"PN's V_element "<<V_element<<endl;
	exit(11);
	*/

	cmatrix Htot(size_basis, size_basis);
	for (int i=0;i<size_basis;i++) {
		v0(i) = complex(1.0,0.0);
		u1 = Hv(jkem, size_grid, HrotKe, Vpot, basisJKeMRe, basisJKeMIm, v0);
		for (int j=0;j<size_basis;j++) {
			Htot(i,j)=u1(j);
		}
		v0(i) = complex(0.0,0.0);
	}                  
	...
	for (int i=0; i<jkem; i++) {
		for (int j=0; j<jkem; j++) {
			...
			for (int ip=0; ip<jkem; ip++) {
				for (int jp=0; jp<jkem; jp++) {
					int jj = jp+ip*jkem;
					if (abs(Htot(ii, jj)) > 10e-2) 
					{
						cout<<"L vec Rotor1: "<<jkemQuantumNumList(i,0)<<" "<<jkemQuantumNumList(i,1)<<" "<<jkemQuantumNumList(i,2)<<endl;
						cout<<"R vec Rotor1: "<<jkemQuantumNumList(ip,0)<<" "<<jkemQuantumNumList(ip,1)<<" "<<jkemQuantumNumList(ip,2)<<endl;
						cout<<"L vec Rotor2: "<<jkemQuantumNumList(j,0)<<" "<<jkemQuantumNumList(j,1)<<" "<<jkemQuantumNumList(j,2)<<endl;
						cout<<"R vec Rotor2: "<<jkemQuantumNumList(jp,0)<<" "<<jkemQuantumNumList(jp,1)<<" "<<jkemQuantumNumList(jp,2)<<endl;
						cout<<"Potential: Real "<<real(Htot(ii, jj))<<endl;
						cout<<"Potential: Imag "<<imag(Htot(ii, jj))<<endl;
						cout<<endl;
					}
				}
			}
	...
	*/
	...
	vector ev=hermdl(Htot);
	//vector ev=diag(Htot);
    ofstream eigenout(fname2.c_str());
    for (int i = 0; i < 5; i++)
    {
        eigenout<<ev(i)<<"   ";
	}
