...
// Generation of names of output file //
	...
	string fname = "lanc-omp-2-p-H2O-jmax" + jprefix.str()+"-Rpt"+rprefix.str() + "Angstrom-grid-"+thetaprefix.str()+"-"+phiprefix.str()+"-niter"+iterprefix.str()+".txt";
...
#pragma omp parallel for private (temp1re, temp1im)
	for (int ii=0;ii<(jkem*size_grid);ii++) {
		int iit=ii;
		int i1=iit/jkem;
		iit=iit-i1*jkem;
		int ig2=iit;
...
#pragma omp parallel for private (temp2re, temp2im)
	for (int ii=0;ii<(size_grid*size_grid);ii++) {
		int iit=ii;
		int ig2=iit/size_grid;
		iit=iit-ig2*size_grid;
		int ig1=iit;
...
#pragma omp parallel for private (temp2re, temp2im)
	for (int ii=0;ii<(size_grid*size_grid);ii++) {
		int iit=ii;
		int ig1=iit/size_grid;
		iit=iit-ig1*size_grid;
		int ig2=iit;

		temp2re(ii)=Vpot(ig1,ig2)*temp2re(ii);
		temp2im(ii)=Vpot(ig1,ig2)*temp2im(ii);
	}
	/*
	for (int ig1=0; ig1<size_grid; ig1++) {
		for (int ig2=0; ig2<size_grid; ig2++) {
...
#pragma omp parallel for private (vec)
	for (int i2=0; i2<jkem; i2++) {
		for (int i1=0; i1<jkem; i1++) {
			for (int ig1=0; ig1<size_grid; ig1++) {
