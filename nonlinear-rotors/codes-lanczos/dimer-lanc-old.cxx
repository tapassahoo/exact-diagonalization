...
void get_Hrot(double ah2o, double bh2o, double ch2o, int jkem, matrix &jkemQuantumNumList, matrix &HrotKe);
...
void get_JKMbasis(int jkem, int size_theta, int size_phi, matrix &dJKeM, matrix &KJKeMRe, matrix &KJKeMIm, matrix &MJKeMRe, matrix &MJKeMIm, matrix &basisJKeMRe, matrix &basisJKeMIm);
void check_normJKeM(int jkem, int size_theta, int size_phi, matrix &basisJKeMRe, matrix &basisJKeMIm, matrix &jkemQuantumNumList); 
...
vector Hv(int jkem, int size_grid, matrix &HrotKe, matrix &Vpot, matrix &basisJKeMRe, matrix &basisJKeMIm, vector &v0);
...
int main(int argc,char **argv) 
{
	...
	matrix basisJKeMRe(jkem,size_grid);
	matrix basisJKeMIm(jkem,size_grid);
	get_JKMbasis(jkem, size_theta, size_phi, dJKeM, KJKeMRe, KJKeMIm, MJKeMRe, MJKeMIm, basisJKeMRe, basisJKeMIm);
	//check_normJKeM(jkem, size_theta, size_phi, basisJKeMRe, basisJKeMIm, jkemQuantumNumList); 
	...
	for (int jkm1=0; jkm1<jkem; jkm1++)
	{
		for (int jkmp1=0; jkmp1<jkem; jkmp1++)
		{
			if ((jkemQuantumNumList(jkm1,0)==jkemQuantumNumList(jkmp1,0)) && (jkemQuantumNumList(jkm1,2)==jkemQuantumNumList(jkmp1,2)))
			{
				...
				{
					HrotKe(jkm1,jkmp1) += 0.25*(ah2o-ch2o)*off_diag(jkemQuantumNumList(jkm1,0),jkemQuantumNumList(jkm1,1))*off_diag(jkemQuantumNumList(jkm1,0),jkemQuantumNumList(jkm1,1)+1);
				}
				else if (jkemQuantumNumList(jkm1,1)==(jkemQuantumNumList(jkmp1,1)+2))
				{
					HrotKe(jkm1,jkmp1) += 0.25*(ah2o-ch2o)*off_diag(jkemQuantumNumList(jkm1,0),jkemQuantumNumList(jkm1,1)-1)*off_diag(jkemQuantumNumList(jkm1,0),jkemQuantumNumList(jkm1,1)-2);
				}
				else if (jkemQuantumNumList(jkm1,1)==(jkemQuantumNumList(jkmp1,1)))
				{
					HrotKe(jkm1,jkmp1) += (0.5*(ah2o + ch2o)*(jkemQuantumNumList(jkm1,0)*(jkemQuantumNumList(jkm1,0)+1)) + (bh2o - 0.5*(ah2o+ch2o)) * pow(jkemQuantumNumList(jkm1,1),2));
				}
			}
		}
	}
		...
		for (int it=0; it<size_theta; it++)
		{
			...
			for (int ip=0; ip<size_phi; ip++)
			{
				...
				for (int ic=0; ic<size_phi; ic++)
				{
					...
					int itpc=ic+(ip+it*size_phi)*size_phi;
					basisJKeMRe(i,itpc)=valdJ*(valMC*valKC-valMS*valKS);//(c1+i*s1)*(c2+i*s2)
					basisJKeMIm(i,itpc)=valdJ*(valMC*valKS+valMS*valKC);
				}
			}
		}
	}
...
void check_normJKeM(int jkem, int size_theta, int size_phi, matrix &basisJKeMRe, matrix &basisJKeMIm, matrix &jkemQuantumNumList) 
{
	...
	for (int itpc=0; itpc<size_theta*size_phi*size_phi; itpc++)
	{
		for (int i=0; i<jkem; i++)
		{
			for (int j=0; j<jkem; j++)
			{
				normJKMRe(i,j)+=basisJKeMRe(i,itpc)*basisJKeMRe(j,itpc)+basisJKeMIm(i,itpc)*basisJKeMIm(j,itpc);
				normJKMIm(i,j)+=basisJKeMRe(i,itpc)*basisJKeMIm(j,itpc)-basisJKeMIm(i,itpc)*basisJKeMRe(j,itpc);
			}
		}
	}
...
vector Hv(int jkem, int size_grid, matrix &HrotKe, matrix &Vpot, matrix &basisJKeMRe, matrix &basisJKeMIm, vector &v)
{
	vector u(jkem*jkem);
	int i1, i1p, i2, i2p;
	...
	for (i2=0;i2<jkem;i2++) {
		for (i1=0;i1<jkem;i1++) {
			for (i1p=0;i1p<jkem;i1p++) {
				u(i1*jkem+i2)+=HrotKe(i1,i1p)*v(i1p*jkem+i2);
			}
		}
	}
	...
	for (i1=0;i1<jkem;i1++) {
		for (i2=0;i2<jkem;i2++) {
			for (i2p=0;i2p<jkem;i2p++) {
				u(i1*jkem+i2)+=HrotKe(i2,i2p)*v(i1*jkem+i2p);
			}
		}
	}
	...
	for (i1=0;i1<jkem;i1++) {
		for (ig2=0;ig2<size_grid;ig2++) {
			for (i2=0;i2<jkem;i2++) {
				temp1re(ig2+i1*size_grid)+=basisJKeMRe(i2,ig2)*v(i2+i1*jkem);
				temp1im(ig2+i1*size_grid)+=basisJKeMIm(i2,ig2)*v(i2+i1*jkem);
			}
		}
	}
	...
	for (ig2=0;ig2<size_grid;ig2++) {
		for (ig1=0;ig1<size_grid;ig1++) {
			for (i1=0;i1<jkem;i1++) {
				temp2re(ig2+ig1*size_grid)+=(basisJKeMRe(i1,ig1)*temp1re(ig2+i1*size_grid)-basisJKeMIm(i1,ig1)*temp1im(ig2+i1*size_grid));
				temp2im(ig2+ig1*size_grid)+=(basisJKeMRe(i1,ig1)*temp1im(ig2+i1*size_grid)+basisJKeMIm(i1,ig1)*temp1re(ig2+i1*size_grid));
			}
		}
	}
	...
	for (ig1=0; ig1<size_grid; ig1++) {
		for (i2=0; i2<jkem; i2++) {
			for (ig2=0; ig2<size_grid; ig2++) {
				temp1re(ig1+i2*size_grid)+=(temp2re(ig2+ig1*size_grid)*basisJKeMRe(i2,ig2)+temp2im(ig2+ig1*size_grid)*basisJKeMIm(i2,ig2));
				temp1im(ig1+i2*size_grid)+=(temp2re(ig2+ig1*size_grid)*basisJKeMRe(i2,ig2)-temp2im(ig2+ig1*size_grid)*basisJKeMIm(i2,ig2));
			}
		}
	}
	...
	for (i2=0; i2<jkem; i2++) {
		for (i1=0; i1<jkem; i1++) {
			for (ig1=0; ig1<size_grid; ig1++) {
				vec(i2+i1*jkem)+=(temp1re(ig1+i2*size_grid)*basisJKeMRe(i1,ig1)+temp1im(ig1+i2*size_grid)*basisJKeMIm(i1,ig1));
			}
		}
	}
	u=u+vec;
