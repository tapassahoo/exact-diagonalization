...
def jobstring(NameOfServer, src_code, Rpt, jmax, size_grid, dir_output, niter):
	logfile = dir_output+"/arpack-submit-p-H2O-Rpt"+str(Rpt)+"ang-j"+str(jmax)+"-grid"+str(size_grid)+"-maxiter"+str(niter)+".txt"
	jobname = "arpack-Rpt"+str(Rpt)

	command_execution = "python " + src_code +"  " +  str(Rpt) + "  " + str(jmax)+"  "+str(niter)
...
if (status == 'S'):
	src_dir = os.getcwd()
	src_code = "/lanczos-arpack-scipy.py"
...
for i in range(nz): 
	...
	if (status == 'S'):
		os.chdir(dir_output)
		#job submission
		fname = 'arpack_Rpt'+str(Rpt)+"Ang"
		fwrite = open(fname, 'w')

		fwrite.write(jobstring(NameOfServer, run_command, Rpt, jrot, grid_increment, dir_output, niter))
		fwrite.close()
	...
	if (status == "A"):
		...
		strFile = "arpack-2-p-H2O-jmax"+str(jrot)+"-Rpt"+str(Rpt)+"Angstrom-grid-"+str(thetaNum)+"-"+str(angleNum)+"-saved-basis.txt"
...
if (status == "A"):
	...
	strFile1 = "arpack-2-p-H2O-jmax"+str(jrot)+"-grid-"+str(thetaNum)+"-"+str(angleNum)+"-saved-basis.txt"
