...
if __name__ == '__main__':    
	...
	strFile = "arpack-2-p-H2O-jmax"+str(Jmax)+"-Rpt"+str(zCOM)+"Angstrom-grid"+str(size_theta)+"-"+str(size_phi)+"-qTIP4P.txt"
				...
				for th2 in range(size_theta):
					for ph2 in range(size_phi):
						for ch2 in range(size_phi):
							...
							v6d[ii,jj]=qpot.caleng(com1,com2,Eulang1,Eulang2)
	...
	count=0
	hv=Hv.Mv(JKeM, size_grid, HrotKe, v6d, basisJKeM, count)
	...
	#np.savetxt(energy_file, vals[::-1], fmt='%8.8e')
	np.savetxt(energy_file, vals, fmt='%8.8e')
